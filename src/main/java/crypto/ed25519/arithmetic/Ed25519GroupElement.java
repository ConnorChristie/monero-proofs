package crypto.ed25519.arithmetic;

import crypto.Scalar;
import jni.CryptoOpsUtil;
 ...
 * [6] Daniel J. Bernstein, Peter Birkner, Tanja Lange and Christiane Peters: Optimizing double-base elliptic-curve
...
public class Ed25519GroupElement implements Serializable {
     ...
     * a = r0 * 2^0 + r1 * 2^1 + ... + r255 * 2^255 with ri in {-15, -13, -11, -9, -7, -5, -3, -1, 0, 1, 3, 5, 7, 9,
     * 11, 13, 15}
     * <br>
        ...
        switch (this.coordinateSystem) {
            ...
            case P3:
                switch (newCoordinateSystem) {
                    ...
                    case CACHED:
                        return cached(this.Y.add(this.X), this.Y.subtract(this.X), this.Z, this.T.multiply
            ...
            case P1xP1:
                switch (newCoordinateSystem) {
                    ...
                    case P3:
                        return p3(this.X.multiply(this.T), this.Y.multiply(this.Z), this.Z.multiply(this.T), this.X
                                .multiply(this.Y));
                    case P1xP1:
    ...
    public void precomputeForScalarMultiplication() {
        ...
        for (int i = 0; i < 32; i++) {
            ...
            for (int j = 0; j < 8; j++) {
                ...
                this.precomputedForSingle[i][j] = precomputed(y.add(x), y.subtract(x), x.multiply(y).multiply
                        (Ed25519Field.D_Times_TWO));
    ...
    public void precomputeForDoubleScalarMultiplication() {
        ...
        for (int i = 0; i < 8; i++) {
            ...
            this.precomputedForDouble[i] = precomputed(y.add(x), y.subtract(x), x.multiply(y).multiply(Ed25519Field
                    .D_Times_TWO));
    ...
    /**
     * Doubles a given group element p in P^2 or P^3 coordinate system and returns the result in P x P coordinate
     * system.
    ...
    public Ed25519GroupElement dbl() {
        switch (this.coordinateSystem) {
            ...
            case P3:
                ...
                return p1xp1(ASquare.subtract(YSquarePlusXSquare), YSquarePlusXSquare, YSquareMinusXSquare, B
                        .subtract(YSquareMinusXSquare));
            default:
                throw new UnsupportedOperationException();
        }
    }
     ...
     * r = this + g where this = (X1 : Y1 : Z1 : T1), g = (g.X, g.Y, g.Z) = (Y2/Z2 + X2/Z2, Y2/Z2 - X2/Z2, 2 * d *
     * X2/Z2 * Y2/Z2)
     * <br>
     * r in P x P coordinate system:
     * <br>
    ...
    public Ed25519GroupElement scalarMultiply(final Ed25519EncodedFieldElement a, boolean useNativeImplementation) {
        ...
        try {
            if (!noNativeLibraryAvailable && this == Ed25519Group.BASE_POINT)
                return new Ed25519EncodedGroupElement(CryptoOpsUtil.scalarMultBase(a.getRaw())).decode();
        ...
        if (precomputedForSingle == null) {
            ...
            if (useNativeImplementation && !noNativeLibraryAvailable) {
                try {
                    return new Ed25519EncodedGroupElement(CryptoOpsUtil.scalarMult(this.encode().getRaw(), a.getRaw()
                    )).decode();
                } catch (UnsatisfiedLinkError e) {
                    System.err.println("WARNING: No C library available for native Ed25519");
                    noNativeLibraryAvailable = true;
