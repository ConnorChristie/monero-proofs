...
public class Keccak {
    ...
    private static final long[] RC = {0x0000000000000001L, 0x0000000000008082L, 0x800000000000808AL,
            ...
            0x800000008000000AL, 0x8000000080008081L,
    ...
    protected int rateSizeBitsFor(int digestSizeBits) {
        switch (digestSizeBits) {
            ...
            case 512:
                return 576;
            default:
                throw new IllegalArgumentException("Invalid digestSizeBits: " + digestSizeBits + " ⊄ { 128, 224, 256," +
                        " 288, 384, 512 }");
        }
    }
    ...
    protected void reset(int rateSizebits, int digestSizeBits) {
        if (rateSizebits + digestSizeBits * 2 != MAX_STATE_SIZE)
            throw new IllegalArgumentException("Invalid rateSizebits + digestSizeBits * 2: " + rateSizebits + " + " +
                    digestSizeBits + " * 2 != " + MAX_STATE_SIZE);
    ...
    public void update(ByteBuffer in) {
        ...
        if ((rateBits & 0x7) > 0) //this could be implemented but would introduce considerable performance
            // degradation - also, it's never technically possible.
    ...
    public void digest(ByteBuffer out) {
        ...
        if (!padded) {
            ...
            if ((rateBits & 0x7) > 0)
                throw new IllegalStateException("Cannot digest while in bit-mode"); //this could be implemented but
            // would introduce considerable performance degradation - also, it's never technically possible.
